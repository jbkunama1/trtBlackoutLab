...
const levels = [
  ...
  },
  {
    ...
    intro: "Wählt zwei Geräte aus, die ihr mit eurer Powerbank betreiben wollt. Berechnet ihre Energie und entscheidet, ob das reicht.",
    type: "text-quiz",
    questionHtml: `
  ...
  },
  {
    ...
    intro: "Ordnet richtig ein: Welcher Bereich braucht im Haushalt am meisten elektrische Energie?",
    type: "single-choice",
    options: [
  ...
  },
  {
    ...
    intro: "Dein 20-kg-Stein liegt 20 m über dem Boden. Welche gespeicherte Energie steckt in ihm?",
    type: "single-choice",
    options: [
  ...
  },
  {
    ...
    intro: "Ihr habt zwei Solarmodule. Entscheidet, wie ihr sie schalten müsst, damit die Lampe mit höherer Spannung leuchtet.",
    type: "single-choice",
    options: [
  ...
  },
  {
    ...
    intro: "Klickt alle Zellen mit der Ziffer 2 an – so erscheint ein Code.",
    type: "number-grid",
    gridSize: 10,
    generateGrid() {
      const rows = [];
...
function renderCurrentLevel() {
  ...
  header.innerHTML = `<span class="house-number">${level.id}</span>`;
...
function handleResult(ok, level, feedback, codeInfo, btnCheck, btnNext) {
  if (ok) {
    ...
    feedback.textContent = "Das passt noch nicht ganz. Versucht es noch einmal.";
    feedback.className = "feedback error";
  }
}
